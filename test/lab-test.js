...
tape("lab(…) exposes l, a and b channel values", function(test) {
  test.labEqual(color.lab("#abc"), 75.104975, -2.292115, -10.528266, 1);
  test.end();
});
...
tape("lab(lab) copies a Lab color", function(test) {
  var c1 = color.lab(50, 4, -5),
      c2 = color.lab(c1);
  test.labEqual(c1, 50, 4, -5, 1);
  c1.l = c1.a = c1.b = 0;
  test.labEqual(c1, 0, 0, 0, 1);
  test.labEqual(c2, 50, 4, -5, 1);
  test.end();
});
...
tape("lab(rgb) converts from RGB", function(test) {
  test.labEqual(color.lab(color.rgb(255, 0, 0)), 53.24079414130722, 80.09245959641109, 67.20319651585301, 1);
  test.end();
});
...
tape("lab(color) converts from another colorspace via color.rgb()", function(test) {
  ...
  test.labEqual(color.lab(new TestColor), 12.65624852526134, 0.12256520883417721, -16.833209795877284, 1);
  test.end();
});

tape("lab.brighter(k) returns a brighter color if k > 0", function(test) {
  var c = color.lab("brown");
  test.labEqual(c.brighter(0.5), 46.52650524281069, 49.69034644081097, 30.543166542619616, 1);
  test.labEqual(c.brighter(1), 55.52650524281069, 49.69034644081097, 30.543166542619616, 1);
  test.labEqual(c.brighter(2), 73.52650524281069, 49.69034644081097, 30.543166542619616, 1);
  test.end();
});
...
tape("lab.brighter(k) returns a copy", function(test) {
  var c1 = color.lab("steelblue"),
      c2 = c1.brighter(1);
  test.labEqual(c1, 52.46551718768575, -4.0774710123572255, -32.19186122981343, 1);
  test.labEqual(c2, 70.46551718768575, -4.0774710123572255, -32.19186122981343, 1);
  test.end();
});
...
tape("lab.brighter() is equivalent to lab.brighter(1)", function(test) {
  var c1 = color.lab("steelblue"),
      c2 = c1.brighter(),
      c3 = c1.brighter(1);
  test.labEqual(c2, c3.l, c3.a, c3.b, 1);
  test.end();
});
...
tape("lab.darker(k) returns a darker color if k > 0", function(test) {
  var c = color.lab("brown");
  test.labEqual(c.darker(0.5), 28.526505242810693, 49.69034644081097, 30.543166542619616, 1);
  test.labEqual(c.darker(1), 19.526505242810693, 49.69034644081097, 30.543166542619616, 1);
  test.labEqual(c.darker(2), 1.5265052428106927, 49.69034644081097, 30.543166542619616, 1);
  test.end();
});
...
tape("lab.darker(k) returns a copy", function(test) {
  var c1 = color.lab("steelblue"),
      c2 = c1.darker(1);
  test.labEqual(c1, 52.46551718768575, -4.0774710123572255, -32.19186122981343, 1);
  test.labEqual(c2, 34.46551718768575, -4.0774710123572255, -32.19186122981343, 1);
  test.end();
});
...
tape("lab.rgb() converts to RGB", function(test) {
  ...
  test.rgbEqual(c.rgb(), 122, 117, 127, 1);
  test.end();
});
