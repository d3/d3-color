...
it("lab(…) returns an instance of lab and color", () => {
  const c = d3.lab(120, 40, 50);
  assert(c instanceof d3.lab);
  assert(c instanceof d3.color);
});

it("lab(…) exposes l, a and b channel values and opacity", () => {
  assertLabEqual(d3.lab("rgba(170, 187, 204, 0.4)"), 74.96879980931759, -3.398998724348956, -10.696507207853333, 0.4);
});
...
it("lab(l, a, b) does not clamp l channel value", () => {
  assertLabEqual(d3.lab(-10, 1, 2), -10, 1, 2, 1);
  assertLabEqual(d3.lab(0, 1, 2), 0, 1, 2, 1);
  assertLabEqual(d3.lab(100, 1, 2), 100, 1, 2, 1);
  assertLabEqual(d3.lab(110, 1, 2), 110, 1, 2, 1);
});

it("lab(l, a, b, opacity) does not clamp opacity to [0,1]", () => {
  assertLabEqual(d3.lab(50, 10, 20, -0.2), 50, 10, 20, -0.2);
  assertLabEqual(d3.lab(50, 10, 20, 1.2), 50, 10, 20, 1.2);
});

it("lab(l, a, b) coerces channel values to numbers", () => {
  assertLabEqual(d3.lab("50", "4", "-5"), 50, 4, -5, 1);
});

it("lab(l, a, b, opacity) coerces opacity to number", () => {
  assertLabEqual(d3.lab(50, 4, -5, "0.2"), 50, 4, -5, 0.2);
});

it("lab(l, a, b) allows undefined channel values", () => {
  assertLabEqual(d3.lab(undefined, NaN, "foo"), NaN, NaN, NaN, 1);
  assertLabEqual(d3.lab(undefined, 4, -5), NaN, 4, -5, 1);
  assertLabEqual(d3.lab(42, undefined, -5), 42, NaN, -5, 1);
  assertLabEqual(d3.lab(42, 4, undefined), 42, 4, NaN, 1);
});

it("lab(l, a, b, opacity) converts undefined opacity to 1", () => {
  assertLabEqual(d3.lab(10, 20, 30, null), 10, 20, 30, 1);
  assertLabEqual(d3.lab(10, 20, 30, undefined), 10, 20, 30, 1);
});

it("lab(format) parses the specified format and converts to Lab", () => {
  assertLabEqual(d3.lab("#abcdef"), 80.77135418262527, -5.957098328496224, -20.785782794739237, 1);
  assertLabEqual(d3.lab("#abc"), 74.96879980931759, -3.398998724348956, -10.696507207853333, 1);
  assertLabEqual(d3.lab("rgb(12, 34, 56)"), 12.404844123471648, -2.159950219712034, -17.168132391132946, 1);
  assertLabEqual(d3.lab("rgb(12%, 34%, 56%)"), 35.48300043476593, -2.507637675606522, -36.95112983195855, 1);
  assertLabEqual(d3.lab("rgba(12%, 34%, 56%, 0.4)"), 35.48300043476593, -2.507637675606522, -36.95112983195855, 0.4);
  assertLabEqual(d3.lab("hsl(60,100%,20%)"), 41.97125732118659, -8.03835128380484, 47.65411917854332, 1);
  assertLabEqual(d3.lab("hsla(60,100%,20%,0.4)"), 41.97125732118659, -8.03835128380484, 47.65411917854332, 0.4);
  assertLabEqual(d3.lab("aliceblue"), 97.12294991108756, -1.773836604137824, -4.332680308569969, 1);
});

it("lab(format) returns undefined channel values for unknown formats", () => {
  assertLabEqual(d3.lab("invalid"), NaN, NaN, NaN, NaN);
});

it("lab(lab) copies a Lab color", () => {
  const c1 = d3.lab(50, 4, -5, 0.4),
      c2 = d3.lab(c1);
...
it("lab(hcl(lab)) doesn’t lose a and b channels if luminance is zero", () => {
  assertLabEqual(d3.lab(d3.hcl(d3.lab(0, 10, 0))), 0, 10, 0, 1);
});

it("lab(rgb) converts from RGB", () => {
  assertLabEqual(d3.lab(d3.rgb(255, 0, 0, 0.4)), 54.29173376861782, 80.8124553179771, 69.88504032350531, 0.4);
});
...
it("lab(color) converts from another colorspace via d3.rgb()", () => {
  ...
  assertLabEqual(d3.lab(new TestColor), 12.404844123471648, -2.159950219712034, -17.168132391132946, 0.4);
});

it("lab.brighter(k) returns a brighter color if k > 0", () => {
  const c = d3.lab("rgba(165, 42, 42, 0.4)");
...
it("lab.brighter(k) returns a copy", () => {
  const c1 = d3.lab("rgba(70, 130, 180, 0.4)"),
      c2 = c1.brighter(1);
