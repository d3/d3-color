...
it("hcl(…) returns an instance of hcl and color", () => {
  const c = d3.hcl(120, 40, 50);
  assert(c instanceof d3.hcl);
  assert(c instanceof d3.color);
});

it("hcl(…) exposes h, c, and l channel values", () => {
  assertHclEqual(d3.hcl("#abc"), 252.37145234745182, 11.223567114593477, 74.96879980931759, 1);
});

it("hcl(…) returns defined hue and undefined chroma for black and white", () => {
  assertHclEqual(d3.hcl("black"), NaN, NaN, 0, 1);
  assertHclEqual(d3.hcl("#000"), NaN, NaN, 0, 1);
  assertHclEqual(d3.hcl(d3.lab("#000")), NaN, NaN, 0, 1);
  assertHclEqual(d3.hcl("white"), NaN, NaN, 100, 1);
  assertHclEqual(d3.hcl("#fff"), NaN, NaN, 100, 1);
  assertHclEqual(d3.hcl(d3.lab("#fff")), NaN, NaN, 100, 1);
});

it("hcl(…) returns undefined hue and zero chroma for gray", () => {
  assertHclEqual(d3.hcl("gray"), NaN, 0, 53.585013, 1);
  assertHclEqual(d3.hcl(d3.lab("gray")), NaN, 0, 53.585013, 1);
});
...
it("hcl(h, c, l) does not wrap hue to [0,360)", () => {
  assertHclEqual(d3.hcl(-10, 40, 50), -10, 40, 50, 1);
  assertHclEqual(d3.hcl(0, 40, 50), 0, 40, 50, 1);
  assertHclEqual(d3.hcl(360, 40, 50), 360, 40, 50, 1);
  assertHclEqual(d3.hcl(370, 40, 50), 370, 40, 50, 1);
});

it("hcl(h, c, l) does not clamp l channel value", () => {
  assertHclEqual(d3.hcl(120, 20, -10), 120, 20, -10, 1);
  assertHclEqual(d3.hcl(120, 20, 0), 120, 20, 0, 1);
  assertHclEqual(d3.hcl(120, 20, 100), 120, 20, 100, 1);
  assertHclEqual(d3.hcl(120, 20, 110), 120, 20, 110, 1);
});

it("hcl(h, c, l, opacity) does not clamp opacity to [0,1]", () => {
  assertHclEqual(d3.hcl(120, 20, 100, -0.2), 120, 20, 100, -0.2);
  assertHclEqual(d3.hcl(120, 20, 110, 1.2), 120, 20, 110, 1.2);
});

it("hcl(h, c, l) coerces channel values to numbers", () => {
  assertHclEqual(d3.hcl("120", "40", "50"), 120, 40, 50, 1);
});

it("hcl(h, c, l, opacity) coerces opacity to number", () => {
  assertHclEqual(d3.hcl(120, 40, 50, "0.2"), 120, 40, 50, 0.2);
});

it("hcl(h, c, l) allows undefined channel values", () => {
  assertHclEqual(d3.hcl(undefined, NaN, "foo"), NaN, NaN, NaN, 1);
  assertHclEqual(d3.hcl(undefined, 40, 50), NaN, 40, 50, 1);
  assertHclEqual(d3.hcl(42, undefined, 50), 42, NaN, 50, 1);
  assertHclEqual(d3.hcl(42, 40, undefined), 42, 40, NaN, 1);
});

it("hcl(h, c, l, opacity) converts undefined opacity to 1", () => {
  assertHclEqual(d3.hcl(10, 20, 30, null), 10, 20, 30, 1);
  assertHclEqual(d3.hcl(10, 20, 30, undefined), 10, 20, 30, 1);
});

it("hcl(format) parses the specified format and converts to HCL", () => {
  assertHclEqual(d3.hcl("#abcdef"), 254.0079700170605, 21.62257586147983, 80.77135418262527, 1);
  assertHclEqual(d3.hcl("#abc"), 252.37145234745182, 11.223567114593477, 74.96879980931759, 1);
  assertHclEqual(d3.hcl("rgb(12, 34, 56)"), 262.8292023352897, 17.30347233219686, 12.404844123471648, 1);
  assertHclEqual(d3.hcl("rgb(12%, 34%, 56%)"), 266.117653326772, 37.03612078188506, 35.48300043476593, 1);
  assertHclEqual(d3.hcl("rgba(12%, 34%, 56%, 0.4)"), 266.117653326772, 37.03612078188506, 35.48300043476593, 0.4);
  assertHclEqual(d3.hcl("hsl(60,100%,20%)"), 99.57458688693686, 48.327323183108916, 41.97125732118659, 1);
  assertHclEqual(d3.hcl("hsla(60,100%,20%,0.4)"), 99.57458688693686, 48.327323183108916, 41.97125732118659, 0.4);
  assertHclEqual(d3.hcl("aliceblue"), 247.7353849904697, 4.681732046417135, 97.12294991108756, 1);
});

it("hcl(format) returns undefined channel values for unknown formats", () => {
  assertHclEqual(d3.hcl("invalid"), NaN, NaN, NaN, NaN);
});

it("hcl(hcl) copies an HCL color", () => {
  const c1 = d3.hcl(120, 30, 50, 0.4),
      c2 = d3.hcl(c1);
...
it("hcl(lab) returns h = NaN if a and b are zero", () => {
  assertHclEqual(d3.hcl(d3.lab(0, 0, 0)), NaN, NaN, 0, 1);
  assertHclEqual(d3.hcl(d3.lab(50, 0, 0)), NaN, 0, 50, 1);
  assertHclEqual(d3.hcl(d3.lab(100, 0, 0)), NaN, NaN, 100, 1);
  assertHclEqual(d3.hcl(d3.lab(0, 10, 0)), 0, 10, 0, 1);
  assertHclEqual(d3.hcl(d3.lab(50, 10, 0)), 0, 10, 50, 1);
  assertHclEqual(d3.hcl(d3.lab(100, 10, 0)), 0, 10, 100, 1);
  assertHclEqual(d3.hcl(d3.lab(0, 0, 10)), 90, 10, 0, 1);
  assertHclEqual(d3.hcl(d3.lab(50, 0, 10)), 90, 10, 50, 1);
  assertHclEqual(d3.hcl(d3.lab(100, 0, 10)), 90, 10, 100, 1);
});

it("hcl(rgb) converts from RGB", () => {
  assertHclEqual(d3.hcl(d3.rgb(255, 0, 0, 0.4)), 40.85261277607024, 106.83899941284552, 54.29173376861782, 0.4);
});
...
it("hcl(color) converts from another colorspace via d3.rgb()", () => {
  ...
  assertHclEqual(d3.hcl(new TestColor), 262.8292023352897, 17.30347233219686, 12.404844123471648, 0.4);
});

it("hcl.brighter(k) returns a brighter color if k > 0", () => {
  const c = d3.hcl("rgba(165, 42, 42, 0.4)");
...
it("hcl.brighter(k) returns a copy", () => {
  const c1 = d3.hcl("rgba(70, 130, 180, 0.4)"),
      c2 = c1.brighter(1);
