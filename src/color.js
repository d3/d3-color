...
var reHex3 = /^#([0-9a-f]{3})$/,
    ...
    reHsvPercent = /^hsv\(\s*([-+]?\d+(?:\.\d+)?)\s*,\s*([-+]?\d+(?:\.\d+)?)%\s*,\s*([-+]?\d+(?:\.\d+)?)%\s*\)$/,
    reHsvaPercent = /^hsva\(\s*([-+]?\d+(?:\.\d+)?)\s*,\s*([-+]?\d+(?:\.\d+)?)%\s*,\s*([-+]?\d+(?:\.\d+)?)%\s*,\s*([-+]?\d+(?:\.\d+)?)\s*\)$/;
...
export default function color(format) {
  ...
  return (m = reHex3.exec(format)) ? (m = parseInt(m[1], 16), new Rgb((m >> 8 & 0xf) | (m >> 4 & 0x0f0), (m >> 4 & 0xf) | (m & 0xf0), ((m & 0xf) << 4) | (m & 0xf), 1)) // #f00
      ...
      : (m = reHsvPercent.exec(format)) ? hsva(m[1], m[2] / 100, m[3] / 100, 1) // hsv(120, 50%, 50%)
      : (m = reHsvaPercent.exec(format)) ? hsva(m[1], m[2] / 100, m[3] / 100, m[4]) // hsva(120, 50%, 50%, 1)
...
function hsva(h, s, v, a) {
  if (a <= 0) h = s = v = NaN;
  else if (v <= 0 || v >= 1) h = s = NaN;
  else if (s <= 0) h = NaN;
  return new Hsv(h, s, v, a);
}

export function hsvConvert(o) {
